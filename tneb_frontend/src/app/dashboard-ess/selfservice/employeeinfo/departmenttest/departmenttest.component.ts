...
@Component({
  selector: 'app-departmenttest',
  templateUrl: './departmenttest.component.html',
  styleUrls: ['./departmenttest.component.scss']
})
export class DepartmenttestComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // this.loader.show();
    this.apiservice.getpersonaldeptdetails(result).then((result)=>{
      // this.loader.hide();
      if(result !=null){
        const apiResponse = result;
        const date=this.convertDateString(apiResponse.startDate1);
        apiResponse.startDate1=date;
        const mergedData = this.mergeCodesAndDescriptions(apiResponse);

        console.log(mergedData);
        this.FamilyForm.push(mergedData);
  ...
  convertDateString(dateStr: string): Date | null {
   ...
   mergeCodesAndDescriptions(data: any): any {
    ...
    const fieldsToMerge = [
      ...
      { code: 'testCode', description: 'departmentTestDetails', mergedField: 'personnelarea_with_description' },
