...
@Component({
  ...
  styleUrls: ['./nominationdetails.component.scss']
})
export class NominationdetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // this.loader.show();
    this.apiservice.getnominationdetails(result).then((result)=>{
      // this.loader.hide();
      if(result !=null){
        ...
        if(mergedData !=null){
          for(let i = 0; i<=mergedData.length- 1;i++){
            const date=this.convertDateString(mergedData[i].birthDate1);
            mergedData[i].birthDate1=date;
          }
        };
        // result.birthdate=this.challengeData ;
        this.FamilyForm=mergedData;
      }
    })
  ...
  convertDateString(dateStr: string): Date | null {
   ...
   mergeCodesAndDescriptions(data: any): any {
    ...
    const fieldsToMerge = [
      { code: 'benefitType', description: 'benefitType_Description', mergedField: 'benefitType_with_description' },
    ];
