...
@Component({
  selector: 'app-personalawards',
  templateUrl: './personalawards.component.html',
  styleUrls: ['./personalawards.component.scss']
})
export class PersonalawardsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.loader.show();
    this.apiservice.getpersonalawarddetails(result).then((result)=>{
      // this.loader.hide();
      if(result !=null){
        this.apiResponse.push(result);

        if(this.apiResponse !=null){
          for(let i = 0; i<=this.apiResponse.length- 1;i++){
            const date=this.convertDateString(this.apiResponse[i].startDate1);
            const date1=this.convertDateString(this.apiResponse[i].dateofJoiningBoa);
            const date2=this.convertDateString(this.apiResponse[i].dateofcompleting2);
            this.apiResponse[i].startDate1=date;
            this.apiResponse[i].dateofJoiningBoa=date1;
            this.apiResponse[i].dateofcompleting2=date2;
          }
          this.FamilyForm=this.apiResponse;
