...
@Component({
  selector: 'app-trainingdet',
  templateUrl: './trainingdet.component.html',
  styleUrls: ['./trainingdet.component.scss']
})
export class TrainingdetComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // this.loader.show();
    this.apiservice.getpersonaltrainingdetails(result).then((result)=>{
      // this.loader.hide();
      if(result !=null){
        ...
        if(this.apiResponse !=null){
          for(let i = 0; i<=this.apiResponse.length- 1;i++){
            ...
            const date1=this.convertDateString(this.apiResponse[i].endDate1);
            this.apiResponse[i].startDate1=date;
            this.apiResponse[i].endDate1=date1;
          }
