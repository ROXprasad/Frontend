...
@Component({
  selector: 'app-personalorder',
  templateUrl: './personalorder.component.html',
  styleUrls: ['./personalorder.component.scss']
})
export class PersonalorderComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    var type='AD';
    // this.loader.show();
    this.apiservice.getpersonalorderdetails(result,type).then((result)=>{
      // this.loader.hide();
      if(result !=null){
        const apiResponse=result;
        if(apiResponse !=null){
          for(let i = 0; i<=apiResponse.length- 1;i++){
            const date=this.convertDateString(apiResponse[i].birthdate);
            result[i].birthdate=date;
          }
        };
        // result.birthdate=this.challengeData ;
        this.additionalCharge=result;
      }
    })
