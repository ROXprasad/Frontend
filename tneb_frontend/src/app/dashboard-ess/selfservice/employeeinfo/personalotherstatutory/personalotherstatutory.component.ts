...
@Component({
  selector: 'app-personalotherstatutory',
  templateUrl: './personalotherstatutory.component.html',
  styleUrls: ['./personalotherstatutory.component.scss']
})
export class PersonalotherstatutoryComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // this.loader.show();
    this.apiservice.getpersonalsantuatorydetails(result).then((result)=>{
      // this.loader.hide();
      if(result !=null){
  ...
  convertDateString(dateStr: string): Date | null {
   ...
   mergeCodesAndDescriptions(data: any): any {
    ...
    const fieldsToMerge = [
      ...
      { code: 'taxationOption', description: 'taxationOption_Description', mergedField: 'personnelarea_with_description' },
